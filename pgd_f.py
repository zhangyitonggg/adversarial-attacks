...
class PGD_F(Attack):
    def __init__(self, model=None, device=None, IsTargeted=None, config=None):
        ...
        @return: None
        """
        super(PGD_F, self).__init__(model, device, IsTargeted)
    ...
    def generate(self, xs=None, ys=None):
        ...
        for _ in range(self.num_steps):
            ...
            all_grad_data = None
            for i,model_ in enumerate(self.model):
                outputs = model_(var_xs)
                ...
                # print(all_grad_data)
                if i != 0:
                    all_grad_data += grad_data
                else:
                    all_grad_data = copy.deepcopy(grad_data)
                    all_grad_sign = copy.deepcopy(grad_sign)
                    

            mask = self.find_mask(all_grad_data)
            copy_xs = copy_xs + self.eps_iter * all_grad_sign * mask
